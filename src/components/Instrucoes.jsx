...
function Instrucoes() {
  // O menu da home utiliza "isDarkMode" e "isMenuOpen"
  ...
  return (
    <div className={`min-h-screen flex justify-center ${isDarkMode ? "bg-[#144774]" : "bg-white"}`}>
      <div className={`w-full max-w-[1440px] min-h-screen ${isDarkMode ? "bg- text-white" : "bg-white text-[#144774]"}`}>
        ...
        <main className="max-w-7xl mx-auto px-4 lg:px-0">
          <h1 className="font-madimione text-center text-2xl lg:text-4xl my-8 lg:my-20">
            VEJA COMO É FÁCIL UTILIZAR O "NOW OR NEVER"
          </h1>

          {[
            {
              titulo: <h2 className={`font-madasemibold text-2xl ${isDarkMode ? "text-white" : "text-[#144774]"}`}>INÍCIO DA JORNADA</h2>,
              subtitulo: <h3 className={`font-madamedium text-1xl mt-2 ${isDarkMode ? "text-white" : "text-[#144774]"}`}>
                Ative o <span className="font-madabold">Now or Never </span>e dê o primeiro passo</h3>,
              texto: <p className={`font-madaregular text-2xl mt-1 text-justify ${isDarkMode ? "text-white" : "text-[#144774]"}`}>Comece sua jornada contra a procrastinação. Ligue seu robô anti-procrastinação pressionando o botão de energia. Em seguida, abra o aplicativo e registre suas tarefas do dia. A Matriz de Eisenhower está integrada ao aplicativo para classificar suas tarefas conforme urgência e importância. Depois, utilize a caneta no display do robô para selecionar uma das tarefas cadastradas e iniciar sua missão.</p>,
              imagem: "./imagens/img-bloco1.avif",
              invertido: false
            },
            {
              titulo: <h2 className={`font-madasemibold text-2xl ${isDarkMode ? "text-white" : "text-[#144774]"}`}>TRABALHO COM PROPÓSITO</h2>,
              subtitulo: <h3 className={`font-madamedium text-1xl mt-1 ${isDarkMode ? "text-white" : "text-[#144774]"}`}>
                Aqui à <span className="font-madabold">sua</span> rotina é prioridade</h3>,
              texto: <p className={`font-madaregular text-2xl mt-2 text-justify ${isDarkMode ? "text-white" : "text-[#144774]"}`}>Trabalhe com inteligência. Ao selecionar uma tarefa, a técnica Pomodoro é ativada e durante os 25 minutos de foco, o LED do robô acende em verde, em seguida, inicia-se o período de descanso de 5 minutos, com o LED mudando para azul. Acompanhe seu progresso pelo aplicativo e ajuste os períodos conforme sua rotina.</p>,
              imagem: "./imagens/img-bloco2.avif",
              invertido: true
            },
            {
              titulo: <h2 className={`font-madasemibold text-2xl ${isDarkMode ? "text-white" : "text-[#144774]"}`}>CONQUISTAS E RECOMPENSAS</h2>,
              subtitulo: <h3 className={`font-madamedium text-1xl mt-1 ${isDarkMode ? "text-white" : "text-[#144774]"}`}>
                Seu esforço é <span className="font-madabold">reconhecido</span> e premiado</h3>,
              texto: <p className={`font-madaregular text-2xl mt-2 text-justify ${isDarkMode ? "text-white" : "text-[#144774]"}`}>Cada tarefa concluída aproxima você da sua melhor versão. A cada tarefa finalizada, você acumula 20 pontos. Ao atingir 100 pontos, o robô desbloqueia uma nova expressão facial e uma mensagem de incentivo personalizada.</p>,
            ...
            },
            {
              ...
              texto: <p className={`font-madaregular text-2xl mt-2 text-justify ${isDarkMode ? "text-white" : "text-[#144774]"}`}>O Now or Never é muito fácil de usar. Assista ao vídeo e entenda como utilizá-lo em conjunto com o aplicativo, uma funcionalidade que nos diferencia.</p>,
              imagem: "./imagens/img-bloco4.jpg",
              invertido: true
            }
          ].map((bloco, i) => (
            <section key={i} className={`flex flex-col lg:flex-row items-center justify-between gap-10 my-12 ${bloco.invertido ? "lg:flex-row-reverse" : ""}`}>
              <div className={`flex-1 text-center ${i === 1 || i === 3 ? 'lg:text-right' : 'lg:text-left'}`}>
                {bloco.titulo && <h2 className={`text-2xl lg:text-4xl font-semibold mb-4 ${isDarkMode ? "text-white" : "text-[#144774]"}`}>{bloco.titulo}</h2>}
                {bloco.subtitulo && <h3 className={`text-xl lg:text-3xl font-light mb-4 ${isDarkMode ? "text-white" : "text-[#144774]"}`}>{bloco.subtitulo}</h3>}
                <p className={`text-base lg:text-lg text-justify ${isDarkMode ? "text-white" : "text-[#144774]"}`}>{bloco.texto}</p>
              </div>
              <div className={`flex-1 mt-6 lg:mt-0 flex ${ i === 0 || i === 2 ? "justify-end" : "justify-start"}`}>
              <img src={bloco.imagem} alt={`Imagem bloco ${i + 1}`} className="w-[480px] rounded-[30px] lg:rounded-[50px]" />
              </div>
            </section>
          ))}
        </main>
